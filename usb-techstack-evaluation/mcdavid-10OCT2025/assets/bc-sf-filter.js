...
var bcSfFilterTemplate = {
    ...
    // Grid Template
    'productGridItemHtml': '<div class="{{itemColumnNumberClass}} {{itemCollectionGroupThumbClass}} thumbnail {{itemCollectionGroupSmallClass}} {{searchClass}}" itemprop="itemListElement" itemscope itemtype="http://schema.org/Product"">' +
...
BCSfFilter.prototype.buildProductGridItem = function(data, index) {
    ...
    var isNew = data.tags.indexOf('new') > -1; 
    ...
    /*** End Prepare data ***/
    //console.log(this.getProductMetafield(data , "bazaarvoice" , "reviews-aggregate-ratings"));
    // Get Template
    var itemHtml = bcSfFilterTemplate.productGridItemHtml;

    var onSaleClass = onSale ? 'sale' : '';
    var soldOutClass = soldOut ? 'OutOfStock' : 'InStock';
    ...
    switch (bcSfFilterConfig.custom.mobile_products_per_row) {
        case 1: itemColumnNumberClass += ' medium-down--one-half small-down--one-whole'; break;
        default: itemColumnNumberClass += ' medium-down--one-half small-down--one-half'; break;
    }
    ...
    var searchClass= '';
    if (this.getProductMetafield(data, 'global', 'brand') !== null) {
    	searchClass = 'search-' + this.getProductMetafield(data, 'global', 'brand');  
   	}
    itemHtml = itemHtml.replace(/{{searchClass}}/g, searchClass);
    ...
    var itemNewLabel = isNew ? bcSfFilterTemplate.newLabelHtml : '';
    itemHtml = itemHtml.replace(/{{itemNewLabel}}/g, itemNewLabel);
	...
	var imageAttr = imageAttr.substring(0, imageAttr.length-1);                                                      
    ...
    if (bcSfFilterMainConfig.general.custom_text || bcSfFilterMainConfig.general.custom_text2 || bcSfFilterMainConfig.general.custom_text3) {
        if (data.tags.includes('custom-text') && bcSfFilterMainConfig.general.custom_text) {
            itemProductInfoHtml += '<div class="custom-text"';
            if(bcSfFilterMainConfig.general.custom_text_color) itemProductInfoHtml += ' style="color: ' + bcSfFilterMainConfig.general.custom_text_color + ';"';
            itemProductInfoHtml += '>';
            if(bcSfFilterMainConfig.general.custom_text_modal) {
                itemProductInfoHtml += '<a href="#" class="open-usb-modal" data-trigger-id="custom-text">';
            } else if(bcSfFilterMainConfig.general.custom_text_url) {
                itemProductInfoHtml += '<a href="'+bcSfFilterMainConfig.general.custom_text_url+'">';
            }
            itemProductInfoHtml += bcSfFilterMainConfig.general.custom_text
            if(bcSfFilterMainConfig.general.custom_text_modal) {
                itemProductInfoHtml += '</a>';
            } else if(bcSfFilterMainConfig.general.custom_text_url) {
                itemProductInfoHtml += '</a>';
            }
            itemProductInfoHtml += '</div>';
        }
        if (data.tags.includes('custom-text2') && bcSfFilterMainConfig.general.custom_text2) {
            itemProductInfoHtml += '<div class="custom-text"';
            if(bcSfFilterMainConfig.general.custom_text2_color) itemProductInfoHtml += ' style="color: ' + bcSfFilterMainConfig.general.custom_text2_color + ';"';
            itemProductInfoHtml += '>';
            if(bcSfFilterMainConfig.general.custom_text2_modal) {
                itemProductInfoHtml += '<a href="#" class="open-usb-modal" data-trigger-id="custom-text2">';
            } else if(bcSfFilterMainConfig.general.custom_text2_url) {
                itemProductInfoHtml += '<a href="'+bcSfFilterMainConfig.general.custom_text2_url+'">';
            }
            itemProductInfoHtml += bcSfFilterMainConfig.general.custom_text2;
            if(bcSfFilterMainConfig.general.custom_text2_modal) {
                itemProductInfoHtml += '</a>';
            } else if(bcSfFilterMainConfig.general.custom_text2_url) {
                itemProductInfoHtml += '</a>';
        ...
        if (data.tags.includes('custom-text3') && bcSfFilterMainConfig.general.custom_text3) {
            itemProductInfoHtml += '<div class="custom-text"';
            if(bcSfFilterMainConfig.general.custom_text3_color) itemProductInfoHtml += ' style="color: ' + bcSfFilterMainConfig.general.custom_text3_color + ';"';
            itemProductInfoHtml += '>';
            if(bcSfFilterMainConfig.general.custom_text3_modal) {
                itemProductInfoHtml += '<a href="#" class="open-usb-modal" data-trigger-id="custom-text3">';
            } else if(bcSfFilterMainConfig.general.custom_text3_url) {
                itemProductInfoHtml += '<a href="'+bcSfFilterMainConfig.general.custom_text3_url+'">';
            }
            itemProductInfoHtml += bcSfFilterMainConfig.general.custom_text3;
            if(bcSfFilterMainConfig.general.custom_text3_modal) {
                itemProductInfoHtml += '</a>';
            } else if(bcSfFilterMainConfig.general.custom_text3_url) {
                itemProductInfoHtml += '</a>';
    ...
    if (typeof comingsoonLabel[0] !== 'undefined') {
        ...
        itemPriceHtml += '<meta itemprop="availability" content="http://schema.org/' + soldOutClass + '" />';
            ...
            if (colorTypes.indexOf(downcasedOption) > -1) {
                ...
                for (var i in data.variants) {
                    ...
                    if (values.indexOf(value) == -1) {
                        ...
                        itemColorSwatchesHtml += 'style="background-image: url(' + this.optimizeImage(variant.image, '400x')  + '); ">';
                        itemColorSwatchesHtml += '</span>';
...
BCSfFilter.prototype.prepareProductData = function(data) {
  ...
  for (var k = 0; k < countData; k++) {
    ...
    if (typeof bcSfFilterConfig.general.currencies != 'undefined' && bcSfFilterConfig.general.currencies.length > 1) {
      ...
      data[k].compate_at_price_max = updateMultiCurrencyPrice(data[k].compate_at_price_max, data[k]['compare_at_price_max_' + currentCurrency]);
    }
